...
export function useSetMarketTradingAC(marketId: string) {
  ...
  const { execute: executeSet, isLoading: isSettingAC, error: setError } = useTransaction({
  ...
  const { execute: executeRemove, isLoading: isRemovingAC, error: removeError } = useTransaction({

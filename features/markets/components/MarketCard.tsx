...
function ResolvedOutcomePill({
  ...
  const winnerName = outcome0Won
    ? (outcomes[0] || 'Yes')
    : (outcomes[1] || 'No');

  return (
    <div className={`flex items-center justify-center gap-1.5 rounded-lg py-2.5 text-sm font-semibold ${
      outcome0Won
...
export function MarketCard({ market }: MarketCardProps) {
  ...
  return (
    <NextLink href={`/market/${market.marketId}`} className="block">
        ...
        <CardHeader className="flex flex-col items-start gap-2 pb-0 flex-1">
          <div className="flex justify-between w-full items-start gap-3">
            <div className="flex items-start gap-2 flex-1">
              <MarketImage metadataURI={market.metadataURI} name={market.question} size="sm" />

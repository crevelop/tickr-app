...
export function ResolutionPanel({ marketId, outcomes, title = 'Resolution', description, bare = false }: ResolutionPanelProps) {
  ...
  const body = isLoading || !status ? (
    ...
    <>
          ...
          <div className="flex flex-col gap-3 mt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Chip size="sm" color="primary" variant="flat">
                {formatCountdown(
                  status.assertionDetails.expirationTime,
                )}
              </Chip>
            </div>
      ...
      {status.phase === 'SETTLED_NOT_REPORTED' && (
        <div className="flex flex-col gap-3 mt-4">
          <p className="text-sm text-default-500">
            The assertion has been settled. Report the resolution to the
            CTF to finalize the market.
          </p>
          <Button
            color="primary"
